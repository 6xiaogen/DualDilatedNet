...
class Teeth3DSDataset(Dataset):
    ...
    def _donwscale_mesh(self, mesh, labels):
        ...
        mesh_simplifier.simplify_mesh(target_count=4000, aggressiveness=3, preserve_border=True, verbose=0,
                                      max_iterations=2000)
        ...
        if faces.shape[0] < 4000:
            fs_diff = 4000 - faces.shape[0]
            faces = np.append(faces, np.zeros((fs_diff, 3), dtype="int"), 0)
        elif faces.shape[0] > 4000:
            mesh_simple = trimesh.Trimesh(vertices=vertices, faces=faces)
            samples, face_index = trimesh.sample.sample_surface_even(mesh_simple, 4000)
    ...
    def __getitem__(self, index):
        if self.in_memory:
            ...
            file = open(join(self.root, self.processed_folder, f), 'rb')
            data = pickle.load(file)  # data tuple 3,  0:[16000,3],   1:[16000,24],  2:[16000]
            if self.post_transform is not None:
                data = self.post_transform(data)
            return data
