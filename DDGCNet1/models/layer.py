...
class DualDilatedEdgeGraphConvBlock(nn.Module):
    ...
    def forward(self, x, pos, cd=None):
        ...
        out2, idx = get_dualgraph_feature(x_t, pos=pos_t, k=self.k_dilated, k_pos=self.k_dilated_pos, idx=idx_fps,farther_bool=False, edge_function=self.edge_function)
        out3 = self.convdilated(out2)
        out4 = torch.max(out3, dim=-1, keepdim=False)[0]
